...
export const IssueDetailsPage = ({ org, repo, issueId, showIssuesList }: IDProps) => {
  ...
  const { comments, commentsError, commentsLoading } = useSelector((state: RootState) => {
    return {
  ...
  if (issue === null) {
    ...
    let renderedComments = comments !== undefined && <IssueComments issue={issue} comments={comments} />
