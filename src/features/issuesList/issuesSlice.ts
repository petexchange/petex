...
const issues = createSlice({
  ...
  reducers: {
    ...
    getIssuesSuccess(state, { payload }: PayloadAction<IssuesResult>) {
      const { pageCount, pageLinks, issues } = payload;
      state.pageCount = pageCount;
      state.pageLinks = pageLinks;
      state.isLoading = false;
      state.error = null;

      issues.forEach(issue => state.issuesByNumber[issue.number] = issue);
...
export const { getIssuesStart, getIssueStart, getIssueFailure, getIssuesFailure, getIssuesSuccess, getIssueSuccess } = issues.actions;
